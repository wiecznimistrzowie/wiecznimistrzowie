  ...
  module EventStore
    ...
    class Client
      ...
      def read(query:, options: ReadOptions.new(sequence_position: 0))
        @event_store
          .read
          .of_type(query.event_types)
          .with_tag(query.tags)
          .after(options.sequence_position)
          # .map { Event.new(**it) }
      end
        ...
        @event_store.append(
          events.map { |event|
            En57::Event.new(
              ...
              data: event.data,
              tags: event.data.map { it.join(":") }
            )
          },
          fail_if: @event_store.read.of_type(condition.event_types).with_tag(condition.tags).after(condition.after)
        )
